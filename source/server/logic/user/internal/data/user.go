...
func (u *UserRepoImpl) FindProfiles(ctx context.Context, uids []int64) ([]*model.User, error) {
	users, err := dal.User.WithContext(ctx).Where(dal.User.UID.In(uids...)).Select(dal.User.UID, dal.User.NickName, dal.User.AvatarURL).Find()
...
func (u *UserRepoImpl) UpdateAvatar(ctx context.Context, accountId, avatar string) error {
	...
	_, err := dal.User.WithContext(ctx).Where(dal.User.AccountID.Eq(accountId)).Updates(
		model.User{
			AvatarURL: avatar,
		},
	)
...
func (u *UserRepoImpl) UpdatePhone(ctx context.Context, uid int64, phone string) error {
	...
	_, err := dal.User.WithContext(ctx).Where(dal.User.UID.Eq(uid)).Updates(
		model.User{
			Phone: phone,
		},
	)
...
func (u *UserRepoImpl) GetAddressList(ctx context.Context) (*v1.AddressList, error) {
	...
	cities, err := dal.City.WithContext(ctx).Find()
	if err = pkg.IsNotRecordNotFoundError(err); err != nil {
		return nil, err
	}
	if len(cities) == 0 {
...
func (u *UserRepoImpl) UpdateUser(ctx context.Context, user *model.User) error {
	...
	_, err := dal.User.WithContext(ctx).Updates(user)
	if err != nil {
		return pkg.InternalError("update user failed: %v", err)
	}
	return nil
}
...
func (u *UserRepoImpl) FindUserByUID(ctx context.Context, uid int64) (*model.User, error) {
	...
	user, err := dal.User.WithContext(ctx).Where(dal.User.UID.Eq(uid)).First()
	if err = pkg.IsNotRecordNotFoundError(err); err != nil {
		return nil, pkg.InternalError("查询失败: %v", err)
	}
	return user, nil
...
func NewUserRepo(data *Data, uidGen *snowflake.Node) biz.UserRepo {
	return &UserRepoImpl{data: data, uidGen: uidGen}
}
